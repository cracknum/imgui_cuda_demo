...
void Shader::setInteger1(int v, const std::string &name)
{
    GLuint location = glGetUniformLocation(getProgramId(), name.c_str());
    glUniform1i(location, v);
}

void Shader::setFloat1(float v, const std::string &name)
{
    GLuint location = glGetUniformLocation(getProgramId(), name.c_str());
    glUniform1f(location, v);
}

void Shader::setFloat3(float a, float b, float c, const std::string &name)
{
    GLuint location = glGetUniformLocation(getProgramId(), name.c_str());
    glUniform3f(location, a, b, c);
}

void Shader::setVector3(const glm::vec3 &vec3, const std::string &name)
{
    GLuint location = glGetUniformLocation(getProgramId(), name.c_str());
    glUniform3fv(location, 1, glm::value_ptr(vec3));
}

void Shader::setVector4(const glm::vec4 &vec4, const std::string &name)
{
    GLuint location = glGetUniformLocation(getProgramId(), name.c_str());
